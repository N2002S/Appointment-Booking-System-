...
class CalendarView:
    ...
    def setup(self, y, m):
        left = Button(self.parent, text='<', command=self.go_prev,bg=bl1,font=("HELVETICA",20,'bold'),fg=blc)
        ...
        right = Button(self.parent, text='>', command=self.go_next,bg=bl1,font=("HELVETICA",20,'bold'),fg=blc)
        ...
        for w, week in enumerate(self.cal.monthdatescalendar(y, m), 2):
            for d, day in enumerate(week):
                btn = Button(self.parent, text=day.strftime('%d'), bg=bl1,width=16,height=4,fg=blc,
...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `user` (user_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, full_name TEXT, username TEXT, password TEXT)")
...
def Register():
    ...
    def register_a():
        ...
        if name.get() == "" or username1.get() == "" or password1.get() == "":
            ...
            if cursor.fetchone() is not None:
                ...
                cursor.execute("INSERT INTO `user` (full_name, username, password) VALUES(?, ?, ?)",(name.get(),username1.get(),password1.get()))
                conn.commit()
    ...
    head = Label(reg,text='NEW USER REGISTRATION',bg=bl, fg=wh, font=('HELVETICA',24,'bold'))
    head.grid(row=0,column=0,padx=10,pady=10,columnspan=2)

    n_lbl = Label(reg,text='Full Name',bg=bl,fg=wh,font=("HELVETICA",14,'bold'))
    ...
    regist = Button(reg,text='Register',font=('HELVETICA',16,'bold'),width=12,fg=blc,bg=bl1,bd=4,relief=RAISED,command=register_a)
    ...
    btn_reg = Button(reg, text="Login", font=('arial',14,'bold'), command=Back2,bg=bl1,fg=blc,width=12,bd=4,relief=RAISED)
...
def HomeWindow():
    ...
    def add_app():
        ...
        confirm = Button(root2,text='Confirm',font=("HELVETICA",14,'bold'),width=13,bg=bl1,fg=blc,command=conf)
    ...
    def edit_app():
        ...
        def selectItem(a):
            ...
            if ask == 'yes':
                ...
                confirm = Button(new,text='Confirm',font=("HELVETICA",14,'bold'),width=13,bg=bl1,fg=blc,command=conf)
                confirm.grid(row=10,columnspan=2,pady=10,padx=10)
    ...
    Home.state('zoomed')
    Home.resizable(0,0)
    head = Label(Home, text="CareConnect", font=('HELVETICA', 20,'bold'),bg=bl,fg=bl1)
    head.grid(row=0,column=0,padx=10,columnspan=7)

    Database()
    cursor.execute("SELECT `full_name` FROM `user` WHERE `username` = ?", (username.get(),))
    ...
    add = Button(Home, text='ADD APPOINTMENT', command=add_app,bg=bl1,fg=blc,width=20,height=2,font=("HELVETICA",15,'bold'))
    add.grid(row=4,column=8,pady=5,padx=(40,10))

    edit = Button(Home, text='EDIT APPOINTMENT', command=edit_app,bg=bl1,fg=blc,width=20,height=2,font=("HELVETICA",15,'bold'))
    edit.grid(row=5,column=8,pady=5,padx=(40,10))

    delete = Button(Home, text='DELETE APPOINTMENT', command=del_app,bg=bl1,fg=blc,width=20,height=2,font=("HELVETICA",15,'bold'))
    delete.grid(row=6,column=8,pady=5,padx=(40,10))  
    
    logout = Button(Home, text='LOGOUT', command=Back,bg=bl1,fg=blc,width=20,height=2,font=("HELVETICA",15,'bold'))
...
btn_login = Button(root, text="Login", font=('arial',14,'bold'), command=Login,bg=bl1,fg=blc,width=12,bd=4,relief=RAISED)
...
btn_reg = Button(root, text="Register", font=('arial',14,'bold'), command=Register,bg=bl1,fg=blc,width=12,bd=4,relief=RAISED)
